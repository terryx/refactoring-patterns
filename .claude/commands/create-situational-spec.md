Create a new refactoring pattern specification in the `src/situational/` directory following this structure:

1. Ask the user for the pattern name (e.g., "Extract Function")
2. Create a directory with kebab-case name (e.g., `extract-function/`)
3. Create `spec.md` inside the directory with ONE example only for clarity
4. Add `## Guideline` section with the pattern's principle text from the "Situational Principles" section in `.claude/claude.md`
...
9. After creating the spec file, add an entry to `README.md` in the "## Supported Patterns" section:
   - Format: `- [x] [Pattern Name](src/situational/[pattern-name]/spec.md)`
...
- **[Pattern Name]**: [Principle text from Situational Principles in .claude/claude.md]
...
- Create directory structure: `src/situational/[pattern-name]/`
- Save spec to `src/situational/[pattern-name]/spec.md`
- Create single consolidated `example.js` in `src/situational/[pattern-name]/example.js`
